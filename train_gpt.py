...
class Hyperparameters:
    ...
    mlp_power = int(os.environ.get("MLP_POWER", 3))
...
class MLP(nn.Module):
    # relu^N MLP — power controlled by mlp_power (default 3, matching original x*x*x)
    def __init__(self, dim: int, mlp_mult: int, mlp_power: int = 3):
        super().__init__()
        hidden = mlp_mult * dim
        self.fc = CastedLinear(dim, hidden, bias=False)
        self.proj = CastedLinear(hidden, dim, bias=False)
        self.proj._zero_init = True
        self.mlp_power = mlp_power

    def forward(self, x: Tensor) -> Tensor:
        x = torch.relu(self.fc(x))
        return self.proj(x ** self.mlp_power)
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = MLP(dim, mlp_mult, mlp_power)
...
def main() -> None:
    ...
    if args.max_val_tokens > 0:
        usable = (
            min(args.max_val_tokens, val_tokens.numel() - 1) // args.train_seq_len
        ) * args.train_seq_len
